...
mod tests {
    ...
    impl TuiState for MockState {
        fn handle_input(&mut self, _input: &str) -> Result<StateTransition, Box<dyn Error>> {
            if self.can_transition {
                Ok(StateTransition::Transition("next".to_string()))
            } else {
        ...
        fn render(&self) -> Result<String, Box<dyn Error>> {
            Ok(format!("Mock state: {}", self.name))
        }
        ...
        fn can_transition_to(&self, target_state: &str) -> bool {
            target_state == "next" && self.can_transition
        }
    }
    ...
    fn test_state_transition() {
        ...
        let state1 = Box::new(MockState {
            name: "state1",
            can_transition: true,
        });
        
        let state2 = Box::new(MockState {
            name: "state2",
            can_transition: false,
        });
        
        manager.add_state("state1".to_string(), state1);
        manager.add_state("state2".to_string(), state2);
        
        manager.set_current_state("state1".to_string()).expect("Failed to set state");
        assert_eq!(manager.current_state_name(), "state1");
        
        let transition = manager.handle_input("test").expect("Failed to handle input");
