...
mod tests {
    ...
    #[test]
    fn test_kanban_add_task() {
        let mut state = KanbanState::new();
        state.add_task("Test task".to_string(), Some("agent1".to_string()));
    ...
    #[test]
    fn test_kanban_move_task() {
        let mut state = KanbanState::new();
        state.add_task("Test task".to_string(), None);
        let task_id = state.tasks[0].id.clone();
        
        state.move_task(&task_id, "doing").expect("Failed to move task");
    ...
    fn test_sessions_add_session() {
        ...
        assert_eq!(state.sessions[0].agent_name, "backend1");
    }
}
