...
mod tests {
    ...
    fn test_help_navigation() {
        ...
        let transition = state.handle_input("down").expect("Failed to handle input");
        assert!(matches!(transition, StateTransition::Stay));
        assert_eq!(state.current_section, 1);
        
        // Test up navigation
        let transition = state.handle_input("up").expect("Failed to handle input");
        assert!(matches!(transition, StateTransition::Stay));
        assert_eq!(state.current_section, 0);
    }
    ...
    #[test]
    fn test_project_select_add_project() {
    ...
    #[test]
    fn test_project_select_navigation() {
        let mut state = ProjectSelectState::new();
        let project = ProjectItem {
            id: "project-1".to_string(),
            name: "test-project".to_string(),
        ...
        let transition = state.handle_input("down").expect("Failed to handle input");
        assert!(matches!(transition, StateTransition::Stay));
        assert_eq!(state.selected_project, Some(0));
    }
}
